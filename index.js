...
/*
  ...
  The nested function can access the internal variable because of closure. Closure happens when inside a scope, some action is taken to grab a variable from outside
  that scope. In this instance, the nestedFunction grabs the internal variable because the variable is one step outside of it's own scope range.
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(zooArr){
    ...
    zooArr.forEach((item) => {
      animNames.push(`name: ${item.animal_name}, scientific: ${item.scientific_name}`)
    })
    return animNames;
  }
  ...
  */
  ...
  function lowerCaseNames(zooArr){
    ...
    const lowerName = zooArr.map((item) => {
      let lowNam = item.animal_name.toLowerCase()
      return lowNam;
    })
    return lowerName;
  }
  ...
  */
  ...
  function lowPopulationAnimals(zooArr){
    ...
    const lowPop = zooArr.filter((elem) => {
      return elem.population < 5
    })
    return lowPop;
  }
  ...
  */
  ...
  function USApop(zooArr){
    let total = zooArr.reduce((acc, curr) => {
